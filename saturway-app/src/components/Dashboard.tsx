...
export function Dashboard({ userName }: DashboardProps) {
  ...
  const stats = [
    ...
    {
      label: t('dashboard.stats.energyLevel'),
      value: '78%',
      percentage: 78,
      icon: Zap,
      color: 'text-[#52C9C1]',
    ...
    {
      label: t('dashboard.stats.focusTime'),
      value: '4.5h',
      percentage: 75,
      icon: Brain,
      color: 'text-[#5AB5E8]',
  ...
  return (
    <div className="space-y-6">
      {/* Hero Section */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#4A9FD8] to-[#52C9C1] p-6 text-white">
        <div className="relative z-10">
          <div className="mb-2 opacity-90">Monday, November 3</div>
          <h1 className="mb-1">
            {t('dashboard.greeting', {
              timeOfDay: t('dashboard.morning'),
              name: userName,
            })}
          </h1>
      ...
      <Card className="border-[#4A9FD8]/20 bg-gradient-to-r from-[#4A9FD8]/5 to-transparent p-5">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 space-y-2">
            <h3 className="text-[#4A9FD8]">{t('dashboard.aiInsight.title')}</h3>
            <p className="text-muted-foreground">
              {t('dashboard.aiInsight.message')}
            </p>
          </div>
        </div>
      </Card>
